document.addEventListener("DOMContentLoaded", async () => {
    ...
    const data = await responceCart.json();
...
function displayCart(products, total) {
    products.map((item) => {
        const tr = document.createElement("tr");
        tr.innerHTML = `<td>${item.id}</td>
                <td><img src="${item.url}" alt="${item.name}"></td>
                <td>${item.name}</td>
                <td>${item.price.toLocaleString()}</td>
                <td><input type="number" value="${item.quantity}" id="${
            item.id
        }quantity" min="1"></td>
...
async function handleCart(table, item) {
    ...
    try {
        ...
        products.map((item) => {
            ...
            const changeQuantity = document.getElementById(
                `${item.id}quantity`
            );
            //input 박스의 변화를 감지하는건 addEventListener("input"
            changeQuantity.addEventListener("input", async () => {
